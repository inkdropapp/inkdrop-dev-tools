  ...
  activate() {
    this.disposables = inkdrop.commands.add(document.body, {
      "dev-tools:copy-notebook-id": () => {
        const bookId =
          inkdrop.store.getState().bookList?.bookForContextMenu?._id;
        logger.debug("Copy notebook id:", bookId);
        clipboard.writeText(bookId);
      },
      "dev-tools:copy-tag-id": (e) => {
        const tagIdForContextMenu =
          inkdrop.store.getState().tagList?.tagForContextMenu?._id;
      ...
      "dev-tools:copy-note-id": () => {
        const noteId =
          inkdrop.store.getState().noteListBar?.actionTargetNoteIds?.[0];
      ...
      "dev-tools:watch-themes": () => {
        ...
        inkdrop.notifications.addInfo("Watching themes...", {
          dismissable: true,
          detail:
            "When any file changes are detected, it will reload immediately.",
        });
      },
